      ...
      <div class="header">
        <div class="header-left">
          <div class="variant-switcher">${(() => {
            ...
            ${SITE_VARIANT === 'health' ? `<span class="variant-divider"></span>
            <a href="#"
               class="variant-option active"
               data-variant="health"
               title="Health Monitor Italia/Europa (attivo)">
              <span class="variant-icon">🩺</span>
              <span class="variant-label">Salute IT/EU</span>
            </a>` : ''}`;
          })()}</div>
          <span class="logo">${SITE_VARIANT === 'health' ? 'HEALTH MONITOR' : 'MONITOR'}</span><span class="version">v${__APP_VERSION__}</span>${BETA_MODE ? '<span class="beta-badge">BETA</span>' : ''}
          ${SITE_VARIANT === 'health' ? `
          <a href="https://github.com/EleCor79" target="_blank" rel="noopener" class="github-link" title="GitHub @EleCor79">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"/></svg>
            <span class="credit-text">@EleCor79</span>
          </a>` : `
          ...
          <div class="region-selector">
            <select id="regionSelect" class="region-select">
              ...
              <option value="it-center" data-lat="41.9" data-lng="12.5" data-zoom="5">🇮🇹 Italia</option>
        ...
        <div class="map-section" id="mapSection">
          <div class="panel-header">
            <div class="panel-header-left">
              <span class="panel-title">${SITE_VARIANT === 'health' ? '🩺 Mappa Sanità Italia/EU' : SITE_VARIANT === 'tech' ? t('panels.techMap') : SITE_VARIANT === 'happy' ? 'Good News Map' : t('panels.map')}</span>
            </div>
